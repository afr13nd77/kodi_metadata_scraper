...
**Версия проекта:** 3.18.1 / 3.18.0 (movie / TV)
**Обновлён:** 26.07.2026
...
| BL-45 | Стоп-слова для очистки названия | `utils.py`, `settings_manager.py` | ~~Базовая часть решена в BUG-009 (v3.18.1)~~ — `clean_title()` теперь автоматически удаляет технические теги (рипы, кодеки, разрешения, HDR, IMAX и др.). Остаётся: пользовательский список стоп-слов через настройки для edge cases. |
