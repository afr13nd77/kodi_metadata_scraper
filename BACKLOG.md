...
| BL-45 | Стоп-слова для очистки названия | `utils.py`, `settings_manager.py` | ~~Базовая часть решена в BUG-009 (v3.18.1)~~, release group names решены в BUG-010 (v3.23.1) — `clean_title()` теперь автоматически удаляет технические теги (рипы, кодеки, разрешения, HDR, IMAX и др.). Остаётся: пользовательский список стоп-слов через настройки для edge cases. |
...
| BL-71 | ✅ Strip коллекционного prefix-а | `utils.py` | `_COLLECTION_PREFIX_PATTERN` + Step 3.0d в `clean_title()`. Strip `[A-Z]{2,5}\d{1,3}-` в начале строки при наличии года в скобках (guard от false positive). 17 тестов. Спецификация: `docs/BL-71_collection-prefix/`. |
...
| BL-66 | ✅ setOriginalLanguage() — Kodi v22 | `kinopoisk_api.py`, `models.py`, `scraper.py`, `tv_scraper.py`, `nfo_writer.py`, `nfo_parser.py` | Маппинг ~45 стран → ISO 639-1. `country_to_language()` из первой страны KP API. `setOriginalLanguage()` с graceful degradation для Kodi < v22. NFO roundtrip через `<language>`. Спецификация: `docs/BL-66_original-language/`. |
| BL-64 | ✅ Режиссёры и сценаристы эпизодов из TVMaze | `tvmaze_client.py`, `tv_scraper.py`, `models.py` | TVMaze API `/episodes/{id}/guestcrew` → `get_episode_crew()` с in-memory кэшем (до 50 записей). `setDirectors()` / `setWriters()` на уровне эпизода. Работает при `use_tvmaze=true`. Graceful degradation при ошибках API. Спецификация: `docs/BL-64_episode-crew/`. |
...
| BL-38 | ✅ Статус сериала (setTvShowStatus) | `models.py`, `tvmaze_client.py`, `kinopoisk_api.py`, `tv_scraper.py`, `nfo_writer.py`, `nfo_parser.py` | 3-уровневый resolve: TVMaze → KP productionStatus → KP completed. NFO roundtrip через `<status>`. Спецификация: `docs/BL-38_tv-show-status/`. |
...
| BL-41 | ✅ Превью эпизодов из TVMaze (addAvailableArtwork) | `tvmaze_client.py`, `tv_scraper.py` | TVMaze episodes API → `addAvailableArtwork(url, "thumb", preview)`. Zero extra HTTP requests (переиспользование _episodes_cache). Спецификация: `docs/BL-41_episode-thumbnails/`. |
| BL-67 | ✅ Рейтинг эпизода (KP + IMDB) | `kinopoisk_api.py`, `tv_scraper.py`, `omdb_client.py`, `models.py` | `Episode.rating` из KP API + `imdbVotes` из OMDb. `setRatings()` с двойным рейтингом (kinopoisk + imdb), preferred source из настроек. Спецификация: `docs/BL-67_episode-rating/`. |
...
| # | Название | Причина |
|---|:---|:---|
| BL-59 | ~~Совместимость с STRM файлами (Elementum / LibreELEC)~~ | `решено` — root cause оказался не в STRM, а в IMDB→KP lookup. Исправлено в BUG-007 / v3.17.1. Скрапер вызывается для .strm корректно. Обратной связи о повторении нет. |

### 6.3 Реализовано

| # | Название | Файл / модуль | Описание |
|---|:---|:---|:---|
| BL-68 | ✅ FanArt.tv артворк фильмов | `shared/fanart_client.py`, `scraper.py`, `settings_manager.py` | FanArt.tv API v3 — clearlogo, clearart, banner, landscape, discart. Новый `FanartClient` с in-memory кэшем (20 записей), rate limiter (1 req/s). Graceful degradation при недоступности. Настройки `use_fanart`, `fanart_api_key`. Спецификация: `docs/BL-68-69_fanart-tv/`. |
| BL-69 | ✅ FanArt.tv артворк сериалов и сезонов | `shared/fanart_client.py`, `shared/tvmaze_client.py`, `tv_scraper.py` | Show-level: clearlogo, clearart, banner, landscape, characterart. Season-level: banner, landscape. TVDB ID через TVMaze `externals.thetvdb`. `addAvailableArtwork(url, type, season=N)`. Спецификация: `docs/BL-68-69_fanart-tv/`. |
...
| BL-72 | Публикация в официальный репозиторий Kodi | `addon.xml`, `icon.png`, `build_zip.py`, `changelog.txt` | Подготовка обоих аддонов к PR в xbmc/repo-plugins: fix icon transparency, дополнить addon.xml (license, platform, source, forum, news), создать changelog.txt, xbmc.python 3.0.1, очистка мусорных файлов, тема на Kodi Forum. |
...
|:---|:---|:---|
| ✅ Реализовано | 40 | BL-01..BL-11, BL-13..BL-20, BL-22..BL-26, BL-35, BL-36, BL-38, BL-40, BL-41, BL-56, BL-57, BL-60, BL-61, BL-64, BL-66..BL-71 |
