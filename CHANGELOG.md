...
### Добавлено
- **BL-39: Ротация API-ключей Кинопоиска (до 5 ключей)** — поддержка до 5 API-ключей KP с автоматической ротацией при HTTP 402 (квота исчерпана) и 403 (невалидный ключ). Модуль-уровневый пул ключей (`init_key_pool`, `rotate_key`, `get_current_api_key`), обёртка `_request_with_rotation` в KinopoiskClient, toast-уведомление при исчерпании всех ключей. Ранний выход из `_handle_find` при исчерпании всех ключей. Обратная совместимость: 1 ключ = прежнее поведение.

### Исправлено
- **Числовые названия фильмов (1917, 2001, 1984, 2012)** — `clean_title()` больше не стирает title, когда он совпадает с bare year pattern. Файл "1917 (2019).mkv" теперь сканируется корректно.
- **Toast при каждом сканировании** — убрано навязчивое уведомление при переключении ключей; toast остаётся только при исчерпании ВСЕХ ключей.
- **Единый rate limiter KP API** — два независимых лимитера (18+9=27 req/s) заменены одним (по умолчанию 20 req/s). Устраняет 429-ошибки при batch-сканировании, которые сжигали дневную квоту retry-запросами. Лимит вынесен в настройку «KP API rate limit».
- **`search_kp_by_imdb` участвует в ротации** — вместо прямого `settings.kinopoisk_api_key` использует `get_current_api_key()` из пула ротации.

### Тесты
- 946 тестов (663 movie + 231 TV + 52 shared). 0 new failures.
...
### Исправлено
- **BUG-010: clean_title() не удаляла release group name** — имена файлов в scene-формате (например `Movie.2025.Quality.Codec-GROUP`) не находились в KP API из-за суффикса release group (`- GROUP` или `   GROUP`), который оставался в поисковом запросе. Добавлен `_RELEASE_GROUP_PATTERN` для удаления одиночного латинского слова после 2+ пробелов (сигнал удалённого года). Легитимные подзаголовки с одинарным пробелом не затронуты.

### Тесты
- 873 теста (615 movie + 224 TV + 34 shared). 0 new failures.
...
- По умолчанию — русский (обратная совместимость, без изменений для существующих пользователей).

### Тесты
- 736 тестов (588 movie + 148 TV). +14 новых тестов (5 models + 2 nfo_writer + 4 scraper + 3 tv_scraper).
