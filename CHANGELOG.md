...
### Добавлено
- **BL-39: Ротация API-ключей Кинопоиска (до 5 ключей)** — поддержка до 5 API-ключей KP с автоматической ротацией при HTTP 402 (квота исчерпана) и 403 (невалидный ключ). Модуль-уровневый пул ключей (`init_key_pool`, `rotate_key`, `get_current_api_key`), обёртка `_request_with_rotation` в KinopoiskClient, toast-уведомления при переключении/исчерпании. Ранний выход из `_handle_find` при исчерпании всех ключей. Обратная совместимость: 1 ключ = прежнее поведение.

### Тесты
- 941 тест (659 movie + 231 TV + 51 shared). 0 new failures. 17 новых тестов для ротации ключей.
